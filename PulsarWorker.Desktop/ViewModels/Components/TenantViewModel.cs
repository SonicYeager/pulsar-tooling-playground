...
public sealed class TenantViewModel : ViewModelBase
{
    ...
    public TenantViewModel(PulsarModel model, IServiceProvider provider)
    {
        _provider = provider;
        this.WhenActivated(disposables =>
        {
            Observable.StartAsync(async () => await model.GetNameSpaces(Name, Notify))
    ...
    private void LoadAsync(IEnumerable<NameSpaceViewModel> viewModels)
    {
        foreach (var cluster in viewModels)
        {
            NameSpaces.Add(cluster);
        }
    }
